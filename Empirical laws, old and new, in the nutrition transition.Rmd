...
Both Engel's Law and Bennett's Law express a log-linear relation between a particular budget and demand for a particular good. In both cases, demand for the particular good in question increases less than proportionately with income. Is there something unique about these goods---food and carbohydrates---or can Engel's and Bennett's Laws be generalized to a single law governing the income elasticity of demand for any good?

Food expenditures to the total budget. Expenditures on carbohydrate-rich foods to the food budget. Carbohydrate expenditures are to the food budget what food expenditures are to the total budget. This symmetry suggests a more general law governing the income elasticity of demand for any good, of which Engel's and Bennett's Laws are but two instances. Analogous laws for other categories of goods were part of the early discussion, clothing...rent, fuel, and light.

At first glance, it may seem that food is unique in that certain biophysical constraints intrinsically limit the amount of this good the human organism can make use of in any given time period. However, the same can be said of toilet paper and exercise equipment. Different, but perfectly analagous, constraints intrinsically limit the usefulness of glue, diamonds, and vacations. This is called the Law of Diminishing Marginal Utility, and it applies to everything, not just food.

Astute observers have documented this law, in one guise or another, throughout much of written history [@kauder1953genesis]. In 1738, Daniel Bernoulli put it this way:
...
This also shines a light on the circumstances under which Engel's and Bennett's Laws could be violated. One can conceive of situations of extreme poverty and/or of soaring prices, for example, such that $\beta$ becomes greater than $1$ for a wide range of goods, including food. Early proponents of Engel's Law were confounded by numerous micro-surveys indicating a more than proportionate increase in food expenditures per increase in income (i.e., a $\beta$ greater than $1$) [@zimmerman1932ernst]. The derivation here suggests that such counterexamples need not be interpreted as a refutation of the law. and offers an indication of    The fleeting discussion of analogous laws for other goods need not have been abandoned due to inconsistent values for $\beta$.

The law of income elasticity of demand derived in equation \ref{eq:general_law} is not about a particular value of $\beta$ for a particular good $x$, nor even whether  but rather about log-linear relation.

[earlier: graph CPI volatility (monthly) against Adj R2, geographically and temporally...then here set up nice segue from that to the new empirical law below.]
...
The variation in parameter values across geographic regions reflects this [inferior-superior good relativity].
...
<!-- # #ggsave("Engels Law 2 Disaggregated.png", width = 8.5, height = 6.5, units = "in") -->
<!-- ``` -->
<!-- Cirera and Masset recently explored the influence of income distribution on Engel's Law [-@cirera2010income], finding that food demand increases as income inequality decreases. A regression of income level and the consumption Gini index supports this finding, and increases the adjusted R-squared by 0.10. -->
<!-- #  -->
<!-- # ```{r} -->

<!-- # df <- read.csv("Engels Law regression.csv", stringsAsFactors = F) -->
<!-- # df$X <- NULL -->
<!-- # colnames(df)[-1] <- c("Food supply (kcal/capita/day), logged", -->
<!-- #                       "GDP / capita (USD), logged", -->
<!-- #                       "Consumption inequality (Gini index)") -->
<!-- # # df2 <- read.csv("Engels Law 2.csv", stringsAsFactors = F) -->
<!-- # # df2$X <- NULL -->
<!-- # # colnames(df2)[3:4] <- c("GDP / capita (USD), logged", -->
<!-- # #                         "Real expenditures / capita (USD), logged") -->
<!-- # # df2$`GDP / capita (USD), logged` <- NULL -->
<!-- # # # df_mod <- merge(df_plot[, c("Country", "Food expenditure share of GDP, logged")], df, by = "Country") -->
<!-- # # df_mod <- merge(df, df2, by = "Country") -->
<!-- # # df_mod <- df_mod[, c("Real expenditures / capita (USD), logged", -->
<!-- # #                      "GDP / capita (USD), logged", -->
<!-- # #                       "Consumption inequality (Gini index)")] -->
<!-- # df_mod <- df[, -1] -->
<!-- # mod <- lm(`Food supply (kcal/capita/day), logged` ~., df_mod) -->
...
<!-- # ```{r Engel2Disagg, fig.show = "hold", fig.width = 6, fig.height=4, fig.align="center", fig.cap="Engel's Law by region, ICP data for 2017.", echo = FALSE} -->
<!-- # df_plot <- df_engel2 -->
<!-- # region_vec <- unique(df_plot$Region) -->
<!-- # facet_labels <- c() -->
<!-- # for(i in 1:length(region_vec)){ -->
<!-- #   this_region <- region_vec[i] -->
<!-- #   this_df_plot <- subset(df_plot, Region == this_region) -->
<!-- #   mod <- lm(`Food demand (kcal/capita/day), logged` ~ `GDP / capita (USD), logged`, this_df_plot) -->
<!-- #   #summary(mod) -->
<!-- #   m <- round(mod$coefficients[2], 2) -->
<!-- #   b <- round(mod$coefficients[1], 2) -->
<!-- #   df_out <- as.data.frame(broom::glance(mod)) -->
