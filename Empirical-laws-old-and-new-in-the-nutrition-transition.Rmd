...
```{r Figure 1, fig.show = "hold", fig.width = 5, fig.height=4, fig.align="left", fig.cap="\\label{fig:dietDiversity}Diets are diverse. Source: Author's creation based on the FAO Food Balance Sheets.", echo = FALSE}
...
Bennett presented this finding not in terms of a regression analysis or a scatterplot of country income against country cereal and starchy root intake, but rather as a world map, with the 34 countries for which he had data shaded according to their respective cereal and starchy root intake. In subsequent work, he extended this to 52 countries, and presented results in a bar chart [@bennett1941wheat]. Drawing on the FAO FBS, it is now possible to extend Bennett's analysis to between 144 and 170 countries and/or economic zones, depending upon the year of analysis. A scatterplot of logged country cereal and starchy root intake against logged country income (GDP) per capita for 2017 in Figure \ref{fig:BennettsLaw} confirms the inverse relation he documented. Each point in this plot represents a country. (See Appendix \ref{sec:noteData} for details regarding the FAO FBS and GDP data, and Appendix \ref{sec:countryGroups} for country groupings.) This empirical relation, now known as Bennett's law, indicates that, regardless of differences in culture, climate, and preference, cereals and starchy roots constitute a declining portion of the diet as income rises.
...
```
The log-linear formulation of Bennett's law fits the FBS data well^[In this and all subsequent regressions in this article, $p << 0.01$ for the slope and y intercept parameters.], especially since 1980; and the fitted parameters have remained remarkably stable over this period. The slope, which has ranged narrowly from `r mBen_min` to `r mBen_max` (Figure \ref{fig:BennettsLaw}, right panel), may be interpreted as the income elasticity of the starchy staple share of food demand. It indicates the percentage change in the starchy staple food share of a country's diet given a $1\%$ increase in per capita income.
...
```{r Figure 3, fig.show = "hold", fig.width = 5, fig.height=5, fig.align="center", fig.cap="\\label{fig:macronutLaws}(Left) Macronutrient laws, 2017 FBS data. (Right) Slope and y intercept trajectories for the carbohydrate law, 1970-2017. The sample size and model fit (adjusted R-squared) are also displayed for each year.", echo = FALSE}
#===========================================================================
#===========================================================================
...
In the spirit of Hans Rosling [-@rosling2005new], animations of Bennett's law, the carbohydrate law, and the fat-carb frontier are provided in the supplemental materials. These animations offer a striking visualization of the nutrition transition, and of the laws' consistency over time. In particular, it is interesting to observe the progress, regress, fits, and starts of different nations along the frontier. The trajectory of mainland China over the past five decades, from an 80% carb diet to a 60% carb diet, is one of many remarkable examples to behold.
...
\doublespacing
Which is not too far from the slope of the expenditure version of Engel's law presented in the lower left graphic of Figure \ref{fig:EngelsLaw}.

As with the other empirical laws explored above, a linear relation exists between the slope $m$ and y intercept $b$ of Engel's law (see Appendix \ref{sec:mAffineYengel} for graphic).
...
As with Bennett's law and the carbohydrate law, this relation only holds when the parameters are estimated using GDP data at constant prices. The theoretical explanation for this behavior derived for Bennett's law in section \ref{sec:derivLaws} also applies to Engel's law.
...
```{r Figure 6, fig.show = "hold", fig.width = 4, fig.height=3, fig.align="center", fig.cap="\\label{fig:backTestEngBen}Comparison of modeled to historical food and starchy staple demand.", echo = FALSE}
...
The choice of future parameter values is facilitated by the remarkable stability of the historical parameter values over time observed in the right panels of Figures \ref{fig:BennettsLaw} and \ref{fig:EngelsLaw}, and by the linear relation between the empirical laws' respective slope and y intercept parameters, observed in Figure \ref{fig:mAffineY} and Appendix \ref{sec:mAffineYengel}. For estimation of 2050 food demand, the income elasticity of food demand is set to `r round(mEngel_fut, 3)`, reflecting an assumption of continued stability. The y intercept parameter is then calculated as `r round(bEngel_fut, 3)` using equation \ref{eq:mAffineYengel}. For the estimation of 2050 starchy staple demand, the income elasticity of starchy staple demand is set to `r round(mBen_fut, 3)`, the mean of the values since 1980, and the y intercept is then calculated as `r round(bBen_fut, 3)` using equation \ref{eq:mAffineYben}.
...
It is tempting to attribute these empirical laws to some special characteristic of food that sets it apart from other goods. However, this approach runs into problems on both empirical and purely argumentative grounds. Empirically, analogous laws can be discerned for a wide range of goods (Figure \ref{fig:otherLaws}), not just foods. The laws for clothing, utilities, and housing were noted in Engel's time [@zimmerman1932ernst; @stigler1954early].

```{r Figure 9, fig.show = "hold", fig.width = 5, fig.height=6, fig.align="center", fig.cap="\\label{fig:otherLaws}A log-linear relation exists between demand and GDP per capita for all major consumer good categories. World Bank ICP 2017 data for 152 countries. GDP is at the market exchange rate, current prices.", echo = FALSE}
...
Where $k_B$ and $k_i$ are constants of integration. This can then be rearranged into the familiar log-linear expression of Engel's law and the other consumer expenditure laws in Figure \ref{fig:otherLaws}.
...
The derivation of the log linear form of consumer expenditure laws thus also provides a theoretical explanation for the linear relation between empirical law slopes and y intercepts observed in Figure \ref{fig:mAffineY} and Appendix \ref{sec:mAffineYengel}. Note that the negative slope in these Figures implies $k_B < 0$. The fact that the linear relation between $m_i$ and $b_i$ is observed to hold only when GDP is expressed at constant prices suggests, moreover, that the constants $k_B$ and $k_i$ are related to price levels.
...
Aitchison's speculation points to both relative prices and increasing incomes as driving forces behind this evolution. As discussed in section \ref{sec:no2order} above, Houthakker later discards the role of increasing incomes on empirical grounds, narrowing the focus to relative prices:
...
Four food export price series are plotted in the left panel of Figure \ref{fig:priceSeries}. After poring over several such plots, one notices that higher price series are consistently more volatile than lower price series. And one notices this to be true across a wide range of food and non-food agricultural commodities, including both raw and processed, aggregate and disaggregate versions of the commodities. The relation between price level and volatility also appears to persist over a wide range of food price epochs, from the oil crisis of 1973 through to the more recent volatile period beginning in 2007. One notices this also appears to be true for farmgate prices and revenue yields (right panel, Figure \ref{fig:priceSeries}). A plot of the logged means of these series against their respective logged standard deviations in Figure \ref{fig:riskReward} confirms a distinct log linear relation between expected value and volatility.^[See Appendix \ref{sec:commodGroups} for details on the commodity grouping in this plot.]

```{r Figure 11, fig.show = "hold", fig.width = 6, fig.height=4, fig.align="center", fig.cap="\\label{fig:riskReward}The log linear relation between the means and standard deviations of (left) world export price, (right top) world farmgate price, and (right bottom) world revenue yield of agricultural commodities. The statistics are taken over all years of available data (over 1961-2018 in the left panel, and over 1991-2018 in the right panel). The right panel includes primary products only (i.e., no processed products), and excludes animal products.", echo = FALSE}
...
GDP data used in the analysis of Bennett's law, the macronutrient laws, and the kcal formulation of Engel's law, are also downloaded from FAOSTAT. In these cases, I used a GDP series at constant prices in order to preserve the linear relation between slopes and y intercepts presented in section \ref{sec:derivLaws}. This relation does not hold when GDP at current prices is used. The 2017 GDP data used in the analysis of the budget share and expenditure versions of Engel's law are expressed at current prices, and are downloaded from the World Bank's International Comparison Program.
...
# df_foodTable1 <- data.frame(FnV_vec1,
#                            animal_vec1,
#                            pulNutOil_vec1,
#                            starch_vec1,
#                            sugar_vec1)
# 
# df_foodTable2 <- data.frame(FnV_vec2,
#                            animal_vec2,
#                            pulNutOil_vec2,
#                            starch_vec2,
#                            sugar_vec2)
...
#                      paste("Sugar, other\nN =", n_sugar))
# 
# colnames(df_foodTable1) <- foodGroup_names
# colnames(df_foodTable2) <- foodGroup_names
# 
# df_foodTable1 <- regulartable(df_foodTable1)
# #df_table <- width(df_table, width = 0.75)
# df_foodTable1 <- FitFlextableToPage(df_foodTable1)
# 
# df_foodTable1 %>% #regulartable() %>% autofit() %>%
#   valign(valign = "top", part = "all") %>%
#   align(align = "left", part = "all") %>%
#   fontsize(i = NULL, j = NULL, size = 9, part = "body") %>%
#   fontsize(i = NULL, j = NULL, size = 10, part = "header")
# 
# df_foodTable2 <- regulartable(df_foodTable2)
# #df_table <- width(df_table, width = 0.75)
# df_foodTable2 <- FitFlextableToPage(df_foodTable2)
# 
# df_foodTable2 %>% #regulartable() %>% autofit() %>%
#   valign(valign = "top", part = "all") %>%
#   align(align = "left", part = "all") %>%
#   fontsize(i = NULL, j = NULL, size = 9, part = "body") %>%
#   fontsize(i = NULL, j = NULL, size = 10, part = "header")
...
The following items are included in the right panel of figure \ref{fig:riskReward} but not the left panel.
...
```{r Figure A1, fig.show = "hold", fig.width = 5, fig.height=5, fig.align="center", fig.cap="\\label{fig:whatReplaces1}Relation between major food group dietary shares and GDP/capita, 2017 FBS data.", echo = FALSE}
...
```{r Figure A2, fig.show = "hold", fig.width = 3, fig.height=3, fig.align="center", fig.cap="\\label{fig:mAffineYengel}Linear relation between the slope and y intercept of Engel's law, 1980-2017.", echo = FALSE}
...
```{r Figure A3, fig.show = "hold", fig.width = 5, fig.height = 3, fig.align="center", fig.cap="\\label{fig:backTestMacronut}Comparison of modeled and historical demand for (left) carbohydrate, (middle) fat, and (right) protein calories.", echo = FALSE}
...
```{r Figure A4, fig.show = "hold", fig.width = 5, fig.height = 5, fig.align="center", fig.cap="\\label{fig:heatMapMacronut}Heatmaps of variation in the 2050 demand projection for (left) carbohydrates, (middle) fats, and (right) proteins, with respect to assumptions regarding GDP per capita and the income elasticity of carbohydrate demand.", echo = FALSE}
...
<!-- ## Risk reward frontier disaggregated by region \label{sec:riskRewardByRegion} -->
