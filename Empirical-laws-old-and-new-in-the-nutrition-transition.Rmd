...
abstract: | 
  \singlespacing Modern discussions of dietary transitions and future food demand implicitly rest upon Engel's law and Bennett's law. However, there is disproportionately little explicit reference to, or straightforward graphical representation of, these fundamental laws in this literature. Occasionally, the laws are mischaracterized or rediscovered, suggesting a basic lack of familiarity with their origins and relevance. Here I address this gap with a theoretical and graphical review of the laws, including analysis of their stability over time. Animations (provided as supplemental materials) provide striking confirmation of the laws' persistence through time. I also introduce novel features and corrollaries to the laws not yet considered in the literature, and walk through an example of how the laws can be applied to generate parsimonious estimates of future food demand. I then propose a theoretical framework by which to unify the laws, together with any other analogous consumer expenditure laws, as particular manifestations of the law of diminishing marginal utility. Finally, I present a new empirical law that appears to have gone unnoticed until now: a log-linear relation between the mean prices of agricultural commodities and their respective volatilities.
...
```{r, fig.show = "hold", fig.width = 5, fig.height=4, fig.align="left", fig.cap="\\label{fig:dietDiversity}Diets are diverse. Source: Author's creation based on the FAO Food Balance Sheets.", echo = FALSE}
...
As with the other empirical laws explored above, a linear relation exists between the parameters of Engel's law (see Appendix \ref{sec:mAffineYengel} for graphic).
...
Again, this can be useful when estimating future food demand, reducing the number of estimated parameters in an $n$ year projection from $2n$ to $2 + n$. A theoretical explanation for this relation is derived in section \ref{sec:derivLaws}.
...
It is somewhat peculiar that Timmer should insist on second order variation of food demand, when a constant income elasticity, together with the declining food budget share in Slutzky's "income effect", would be sufficient to explain a declining (absolute) price elasticity of food demand as incomes rise; and would be consistent with Engel's law, rather than at odds with it.

<!-- Timmer, on the other hand, specifies a model allowing for second order variation, but this is his own surmise, not necessarily a "logical corrollary" of Engel's law. -->
...
```{r, fig.show = "hold", fig.width = 4, fig.height=3, fig.align="center", fig.cap="\\label{fig:backTestEngBen}Comparison of modeled to historical food and starchy staple demand.", echo = FALSE}
...
```{r, fig.show = "hold", fig.width = 5, fig.height=6, fig.align="center", fig.cap="\\label{fig:otherLaws}A log-linear relation exists between demand and GDP per capita for all major consumer good categories. World Bank ICP 2017 data for 152 countries. GDP is at the market exchange rate, current prices.", echo = FALSE}
...
\begin{equation}
\begin{split}
\int \frac{\partial U}{\partial x} \: dx &= \int \alpha \frac{d \ln(x)}{dx} \: dx \\
U(x)  &= \alpha \ln(x) + k
\end{split}
\label{eq:integrate_lnU}
\end{equation}

Where the proportionality constant $\alpha = \partial U/ \partial \ln(x)$ regulates the marginal change in utility given a $1$ percent increase in good $x$.
...
\begin{equation}
\begin{split}
\int \nabla_{\mathbf{x}} U \: d \mathbf{x} &= \int D(\boldsymbol{\alpha}) \nabla_{\mathbf{x}} \ln(\mathbf{x}) \: d \mathbf{x} \\
...
\begin{equation}
\nabla_{\mathbf{x}} U = \left[ \begin{matrix} \frac{\partial U}{\partial x_1} \\
...
Factoring out the nominal price $w_i$ and integrating gives
...
where the income elasticity of good $x_i$ (or slope) $m_i$ and y intercept $b_i$ are defined
...
Aitchison's speculation points to both relative prices and increasing incomes as driving forces behind this evolution. As discussed in the previous section, Houthakker later discards the role of increasing incomes on empirical grounds, narrowing the focus to relative prices:
...
Viewed from this perspective, the numerous 19th century working class budget studies mentioned in the previous section [@zimmerman1932ernst] that seem to disprove Engel's law merely present a more complete picture of the evolution from food as luxury to food as an easily accessible household good. During the industrial revolution, most of the world lived in the countryside, where they produced many of their own household goods. The working class who came to the cities, on the other hand, were beholden to the market for the acquisition of such goods. Relative prices placed all but the most basic of goods out of their reach; and food---particularly food of an acceptable quality---was effectively rendered a luxury with respect to the panoply of low quality foodstuffs and other basic goods that were within reach. The present theoretical discussion can scarcely evoke the real significance of such circumstances, so far removed from our own time and experience; and so a memory aid is provided in the form of the following quote from a famous^[Among Marxists.] passage in Volume I of _Capital_, where Marx synthesizes (with typical sarcasm) the results of an 1863 parliamentary inquiry into English working class conditions:
...
The theoretical explanation proposed above implies that estimates of future demand hinge critically on assumptions regarding the future evolution of relative prices. Fortunately, food price volatility has not been an issue for most of the past 47 years. After a tumultuous decade culminating in the oil crisis of 1973, food prices began a long period of steady, stable decline. However, since 2007, staple food prices have again begun to exhibit volatility, severely curtailing access to essential foodstuffs on numerous occasions in many parts of the world. A fledgling discussion of the causes and effects of this volatility has begun [@CWFSpriceVol2011; @imf2011price; @von2012global; @kalkuhl2016food]. Here I present a new empirical finding that may help to further orient this discussion.
...
gg <- ggplot(df_plot, aes(x = Year, y = `World export price (USD / metric ton)`, group = Item, color = Item)) + geom_line()
...
```{r, fig.show = "hold", fig.width = 4, fig.height=4, fig.align="center", fig.cap="\\label{fig:riskReward}Logged mean price of agricultural commodities plotted against their respective logged standard deviations. Source: Author's creation using FAO export data 1961-2017.", echo = FALSE}
...
The law also fits the data well at geographically disaggregated scales (Appendix \ref{sec:riskRewardByRegion}). The slopes at the disaggregated scale are considerably lower than at the globally aggregated level, especially in Asia and Africa. At the globally aggregated level, the mean price of a given agricultural commodity increases roughly proportionately to the increase in volatility, while at the geographically disaggrgated level, mean prices increase less than proportionately with volatility. At disaggregated levels, then, the producer's price risk grows exponentially with expected return.
...
The empirical relations in Appendix \ref{sec:riskRewardByRegion} thus imply that efforts to increase smallholder access to premium and value added markets must go hand in hand with efforts to mitigate the associated increase in price risk. This mitigation requires a great deal of innovation, as smallholders generally have no means by which to accumulate risk capital in the classical sense.
...
df_animal <- data.frame(animal_vec)
...
```{r, fig.show = "hold", fig.width = 3, fig.height=3, fig.align="center", fig.cap="\\label{fig:mAffineYengel}Linear relation between the slope and y intercept of Engel's law, 1980-2017.", echo = FALSE}
