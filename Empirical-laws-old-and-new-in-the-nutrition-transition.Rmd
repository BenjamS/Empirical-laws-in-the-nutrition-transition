...
This implies a marginal increase in the protein share of diet of about `r -(-mFatCarb_mu - 1)` per marginal decrease in the carbohydrate share. The magnitude of the increase in $x_p$ is slight enough to be in virtual agreement with Perisse et al.'s finding of constant protein shares across all income levels. However, it is also large enough to imply a significant percentage increase in global protein intake by 2050, as shown in section \ref{sec:futurDem} below.
...
As with the other empirical laws explored above, a linear relation exists between the parameters of Engel's law (Figure \ref{fig:mAffineYengel} in Appendix).
...
\doublespacing
Here Timmer has in mind the elasticity form of the Slutzky decomposition of the slope of the demand curve for a given good (the "Cournot own-price elasticities") into a "substitution effect" and a so-called "income effect", where the "income effect" is typically interpreted as the change in a consumer's expenditure on the good in question given a change in their income resulting from a change in the good's price. When the good in question is food, this interpretation of Slutzky's "income effect" is congruent to the income elasticity of food demand that appears in Engel's law. Timmer's argument is thus tantamount to arguing that Engel's law strongly suggests that the income elasticity of food demand should decline with income.

This is problematic for two reasons. Firstly, the so-called "income effect" in Slutzky's equation is, more precisely, an optimal expenditure effect, where optimal expenditure is typically a function of prices, technology, and, notably, not income. Optimal expenditure may thus be greater or less than income, and is thus, in the specific case of food, not necessarily congruent to the income elasticity of food demand that appears in Engel's law.

Secondly, as mentioned at the start of this subsection, there is nothing in Engel's law to suggest that the income elasticity of food demand should change with income.
...
\doublespacing
Again, Engel's law implies no such corrollary. Engel leaves the issue of second order behavior to be addressed by the modeler. Engel himself specified a log-linear model, which implies zero second order variation. Houthakker also specifies a log-linear model, with results suggesting very little change in income elasticities of food demand with respect to income across a diverse range of countries and time periods. Houthakker expressly considers the possibility of second order variation, and concludes that "there is not much support for this thesis" [-@houthakker1957international].

Timmer, on the other hand, specifies a model allowing for second order variation, but this is his own surmise, not necessarily a "logical corrollary" of Engel's law. Seale and Regmi [-@seale2006modeling] then go a step further, conflating Timmer's surmise with Engel's law itself.

>\singlespacing "Engel's law states that the budget share of food decreases with increasing income. Accordingly, the budget share of food should be larger in Vietnam than in the U.S., and _the income elasticity of demand for food is expected to be greater in Vietnam than in the U.S._" [@seale2006modeling].

\doublespacing
To be clear, this is an accurate representation of Engel's law up until the last clause (in italics, mine), where the authors misattribute Timmer's surmise to Engel. Valin et al. subsequently complete the metamorphosis of Timmer's surmise from a "logical corrollary" of Engel's law to Engel's law itself:

>\singlespacing "[T]here is overwhelming evidence that income elasticities for most food commodities decline as consumer income increases, a relationship known as Engel's law" [@valin2014future].
...
But note that, assuming other expenditures are held constant, $\frac{d \ln(B)}{d B} = \frac{\partial \ln(B)}{\partial (w_i x_i)}$. And so
...
Where $k_B$ and $k_i$ are constants of integration. This can then be rearranged into the familiar log-linear expression.
...
All of the empirical laws in Figure \ref{fig:otherLaws}, as well as Engel's law, may thus be considered manifestations of the law of diminishing marginal utility (equation \ref{eq:dimUtilDefine}) in conjunction with the consumer's conservation of utility condition (equation \ref{eq:condEqualU}). Bennett's law may likewise be derived if $B$ is constrained to refer to the consumer's food budget, and the goods $x_i$ are constrained to refer to food items.
...
The derivation of the log linear form of consumer expenditure laws thus also provides a theoretical explanation for the linear relation between empirical law slopes and y intercepts observed in Figures and . Note that the negative slope in these Figures implies $k_B < 0$. The fact that the linear relation between $m_i$ and $b_i$ is observed to hold only when GDP is expressed at constant prices suggests, moreover, that the constants $k_B$ and $k_i$ are related to price levels.
...
\doublespacing
In hindsight, Aitchison's remarks regarding "the future prospects of television, domestic refrigerators, and the like" have, of course, proven prescient. The list of goods that have evolved from luxury to necessity status since Aitchison's article can easily be extended: laptop computers, cell phones, laser technology, etc. Looking forward, one might expect the same trajectory of quantum computers, space tourism, "and the like".
...
>\singlespacing "An explanation of the differences between the elasticities [based in income] consequently seems difficult. It is conceivable, and indeed probable, that relative prices may influence the elasticities" [@houthakker1957international].
