...
# Using Engel's Law to estimate future food demand
...
list_DemStarch <- list()
  for(i in 1:n_yrs){
    ...
    worldDemTot <- df_world$`Total (kcal/capita/day)`
    ...
    mod <- lm(`Cereals & starchy roots (kcal/capita/day), logged` ~ `GDP/capita (USD), logged`, df_mod)
    #summary(mod)
    a <- round(mod$coefficients[2], 4)
    b <- round(mod$coefficients[1], 4)
    df_out <- as.data.frame(broom::glance(mod))
