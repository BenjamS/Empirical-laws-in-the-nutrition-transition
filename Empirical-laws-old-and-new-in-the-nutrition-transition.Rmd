...
Food demand studies are highly academic, onerous, opaque affairs... 
...
worldGDP2050_2pct <- worldGDP2017 * pctChng_GDP

worldGDPPcap2050_SSP2 <- 16000
worldGDPPcap2050_FAO <- 11000
...
me_estTrilKcal <- DemTot2050_est * 10^-12
...
# Other food demand estimates (for context, direct comparison problematic due to difference units for GDP.)
# From FAOSTAT
...
modName_vec <- c("AIM", "IMPACT", "GLOBIOM",
                 "MAgPIE", "MAIDADS", "FAO",
                 "Engel-based physical Flow",
                 "Engel's Law",
                 "Engel's Law",
                 "Engel's Law")

modSource_vec <- c(rep("Valin et al. 2014", 4),
...
worldPop_2050SSP2 <- 9.287 * 10^9 # Valin et al. footnote 11
SSP2estTrilKcal_vec <- c(AIM_estTrilKcal, IMPACT_estTrilKcal,
              GLOBIOM_estTrilKcal, MAgPIE_estTrilKcal,
              Gouel_estTrilKcal)
SSP2estKcalPcap_vec <- SSP2estTrilKcal_vec / worldPop_2050SSP2
FAO_estTrilKcal <- kcalPday2005 * 1.54 * 10^-12 #Expert assessment
worldPop_2050FAO <- 9.15 * 10^9 # Valin et al. footnote 11
FAO_estKcalPcap <- FAO_estTrilKcal / worldPop_2050FAO

Tilman_estTrilKcal <- kcalPday2005 * 2.1 * 10^-12 # Econometric
Tilman_estKcalPcap <- Tilman_estTrilKcal / worldPop_2050SSP2

worldPop_UNmedium <- 9.735 * 10^9 #https://population.un.org/wpp/Publications/Files/WPP2019_Highlights.pdf Table 1
Bodirsky_estKcalPcap <- 2990 # Econometric
Bodirsky_estTrilKcal <- Bodirsky_estKcalPcap * worldPop_UNmedium
...
Bijl_estTrilKcal <- Bijl_estKcalPcap * worldPop_2050SSP2

estTrilKcal_vec <- c(SSP2estKcalPcap_vec,
                     FAO_estTrilKcal,
                     Bijl_estTrilKcal,
                     Tilman_estTrilKcal,
...
estKcalPcap_vec <- c(SSP2estKcalPcap_vec,
                     FAO_estKcalPcap,
                     Bijl_estKcalPcap,
                     Tilman_estKcalPcap,
...
popProject_vec <- round(c(rep(worldPop_2050SSP2, length(SSP2estTrilKcal_vec)),
                    worldPop_2050FAO, worldPop_2050SSP2,
                    worldPop_UNmedium,
                    worldPop_2050SSP2, worldPop_UNmedium) * 10^-9, 2)
...
Analogous empirical laws can be said to exist for a wide range of goods (Figure ...). The laws for clothing, utilities, and rent were noted in Engel's time, but are not as popular today [@zimmerman1932ernst; @stigler1954early].
...
At first, it may seem that there is something special about food that sets it apart from other goods. In the early discussion surrounding Engel's Law, analogous laws for clothing, rent, and utilities were also proposed [@zimmerman1932ernst], but did not exhibit the same consistency as Engel's Law, and are hardly discussed today.

Houthakker relative prices explanation.
