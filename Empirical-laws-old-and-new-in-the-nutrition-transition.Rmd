...
This implies an increase in the protein share of diet of about `r -(-mFatCarb_mu - 1)` per marginal decrease in the carbohydrate share. The magnitude of the increase in $x_p$ is slight enough to be in virtual agreement with Perisse et al.'s finding of constant protein shares across all income levels. However, it is also large enough to imply a significant percentage increase in global protein intake by 2050, as discussed below.
...
The question of how overwhelming this evidence may or may not be lies beyond the scope of this review. However, the foregoing reminder of Engel's original formulation of the law and of Houthakker's subsequent empirical work on the subject are effectively submitted, together with Figure \ref{fig:EngelsLaw}, as evidence in favor of the log-linear model, which implies no second order variation in food expenditure with respect to changes in income. If there is a logical corrollary to be drawn from Figure \ref{fig:EngelsLaw}, after all, it is that the income elasticty of food demand is unresponsive to changes in income: Over the period 1980 to 2017, world GDP per capita rose by $328\%$ [@WDIgdp], while the income elasticty in the graphic declined by just two percentage points. [A cursory exploration of simple second order models (as Timmer did except he also included prices) resulted in statistically insignificant coefficients on the squared term].
<!-- from $\$2533$  to $\$10,828$ -->
...
#df_table <- width(df_table, width = 1.25)
...
```{r, fig.show = "hold", fig.width = 5, fig.height=5, fig.align="center", fig.cap="\\label{fig:otherLaws}A log-linear relation exists between demand and GDP per capita for all major consumer good categories. World Bank ICP 2017 data for 152 countries. GDP is at the market exchange rate, current prices.", echo = FALSE}
...
>\singlespacing "An explanation of the differences between the elasticities consequently seems difficult. It is conceivable, and indeed probable, that relative prices may influence the elasticities" [@houthakker1957international].
...
df_foodTable <- regulartable(df_foodTable)
#df_table <- width(df_table, width = 0.75)
df_foodTable <- FitFlextableToPage(df_foodTable)
...
```{r, fig.show = "hold", fig.width = 5, fig.height = 3, fig.align="center", fig.cap="\\label{fig:backTestMacronut}Comparison of modeled and historical demand for (left) carbohydrates, (middle) fats, and (right) proteins.", echo = FALSE}
