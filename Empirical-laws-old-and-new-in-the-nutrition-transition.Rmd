...
colnames(df_bennett)[which(colnames(df_bennett) == "GDP.capita..USD..2015.prices...logged")] <- "GDP/capita (USD), logged"
colnames(df_bennett)[which(colnames(df_bennett) == "GDP.capita..USD..2015.prices.")] <- "GDP/capita (USD)"
...
```
Nonetheless, Bennett found the "way of characterizing summarily the diets of different nations" he was looking for by examining the starchy carbohydrate content of diets in relation to income. An example is given in the left panel of Figure \ref{fig:Bennett01}. Each point in this plot represents a country. The countries are grouped by geographic region (see Appendix for details on the grouping). The empirical relation indicates that, regardless of the vast cultural differences noted by Bennett, carbohydrates constitute a declining portion of the diet as income rises.

Bennett looked at cereals and potatoes in particular. Ratio of cereal-potato intake to total intake of all other foods.

After considering a number of possible explanations for the differences such as climate, preference, and religious taboo, he could not help but notice the correlation between high cereal-potato intake and low income.  "[I]t is difficult to escape the conclusion that explanation of the differences in the degree of dependence on cereal-potato calories is to be found chiefly in the relative economic status of nations or of families" [@bennett1941international].

"[I]rregularities and exceptions [aside], a rough inverse relation between level of income and dependence upon cereal-potato calories seems to exist and to have a rational causal basis, whether comparisons refer to families, localities, or nations."

GDP is based on 2015 prices.
...
these_cols <- c("Area", "Year", "Region", "Element", "Item", "Value",
                "GDP/capita (USD), logged", "GDP/capita (USD)")
...
df_tot <- subset(df_foodShare[, c("Area", "Year", "Item", "GDP/capita (USD), logged", "GDP/capita (USD)", "Value")], Item == "Grand Total")
df_tot$Item <- NULL
...
df_plot <- subset(df_macnut, Year == 2017)
gg <- ggplot(df_plot, aes(x = `GDP/capita (USD), logged`,
                          y = `Diet share (%), logged`,
                          group = Region, fill = Region,
                          shape = Region))
...
# Get parameters
df_mod <- subset(df_macnutShare, Year == 2017)
...
df_plot <- subset(df_macnutShare, Year == 2017)
df_plot$label_these <- NA
u <- df_plot$Area
df_plot$label_these[which(u %in% labelThese_vec)] <- df_plot$Area[which(u %in% labelThese_vec)]
...
colnames(df_cerRootShare)[which(colnames(df_cerRootShare) == "Diet share (%), logged")] <- "Cereals & starchy roots (% of diet), logged"
...
for(i in 1:n_yrs){
  ...
  mFatCarb_vec[i] <- round(mod$coefficients[2], 3)
  bFatCarb_vec[i] <- round(mod$coefficients[1], 3)
  df_out <- as.data.frame(broom::glance(mod))
  adjR2FatCarb_vec[i] <- df_out$adj.r.squared
  #---------------------------------------------------
  mod <- lm(`Cereals & starchy roots (% of diet), logged` ~ `GDP/capita (USD), logged`, this_df)
  mBennett_vec[i] <- round(mod$coefficients[2], 3)
  bBennett_vec[i] <- round(mod$coefficients[1], 3)
  df_out <- as.data.frame(broom::glance(mod))
  adjR2Bennett_vec[i] <- df_out$adj.r.squared

}
df_plotBennett <- data.frame(Year = yr_vec, Slope = mBennett_vec, bBennett_vec, adjR2Bennett_vec)
colnames(df_plotBennett)[3:4] <- c("Y intercept", "Adj. R-squared")
df_plotBennett$Year <- as.integer(as.character(df_plotBennett$Year))

df_plotFatCarb <- data.frame(Year = yr_vec, Slope = mFatCarb_vec, bFatCarb_vec, adjR2FatCarb_vec)
colnames(df_plotFatCarb)[3:4] <- c("Y intercept", "Adj. R-squared")
...
Both Bennett's Law and the fat-carb frontier have fit the data well across time, especially since 1980 (Figure \ref{fig:BennettStability}). Over the period of best fit (1980-present), the slope of Bennett's Law has ranged narrowly between `r minSlopeBennett` and `r maxSlopeBennett`, while the slope of the fat-carb frontier has also ranged narrowly between `r minSlopeFatcarb` and `r maxSlopeFatcarb`. The animations provided in the supplemental materials offer further visual confirmation of this stability.
...
#summary(mod)
  mBen <- round(mod$coefficients[2], 2)
  bBen <- round(mod$coefficients[1], 2)
  df_out <- as.data.frame(broom::glance(mod))
  adjR2Ben <- round(df_out$adj.r.squared, 2)
this_subtitle <- paste0("Adj. R-squared = ", adjR2Ben, "\nSlope = ", mBen, ", Y intercept = ", bBen)

df_plotFC <- subset(df_plotFatCarb, Year >= 1980)
mod <- lm(Slope ~ `Y intercept`, df_plotFC)
#summary(mod)
  mFC <- round(mod$coefficients[2], 2)
  bFC <- round(mod$coefficients[1], 2)
  df_out <- as.data.frame(broom::glance(mod))
  adjR2FC <- round(df_out$adj.r.squared, 2)
this_subtitle <- paste0("Adj. R-squared = ", adjR2FC, "\nSlope = ", mFC, ", Y intercept = ", bFC)


```

Note that the slope and y intercept of Bennett's Law appear to move in inverse unison with each other. A plot of their values reveals they are indeed affine to each other across all years (Figure \ref{fig:mAffineY}). This relation effectively reduces Bennett's Law to a one parameter model when comparing across years. That is, the slope of Bennett's Law $\alpha$ is related to its y intercept $b$ by

\begin{equation}
\alpha \approx `r mBen` b + `r bBen`
\end{equation}

for all years. Likewise for the fat-carb frontier,

\begin{equation}
\alpha \approx `r mFC` b + `r bFC`
\end{equation}

```{r, fig.show = "hold", fig.width = 5, fig.height=3, fig.align="center", fig.cap="\\label{fig:mAffineY}The slopes of Bennett's Law and the fat-carb frontier are linear functions of their respective y intercepts.", echo = FALSE}
...
```{r, fig.show = "hold", fig.width = 4, fig.height=4, fig.align="center", fig.cap="\\label{fig:Engel0}Expenditure formulation of Engel's Law. Source: Author's creation based on World Bank International Comparison Program (ICP) data for 2017.", echo = FALSE}
...
```{r, fig.show = "hold", fig.width = 4, fig.height=3, fig.align="center", fig.cap="\\label{fig:qerwerwer}....", echo = FALSE}
this_file <- "World Bank ICP Data raw.csv"
df_engelOther <- read.csv(this_file, stringsAsFactors = F)
#colnames(df_engelOther)
keep_cols <- c(1, 3, 5, 7)
df_engelOther <- df_engelOther[, keep_cols]
colnames(df_engelOther) <- c("Country", "Element", "Good", "Value")
df_engelOther$Value <- as.numeric(df_engelOther$Value)
#unique(df_engelOther$Element)
element_vec <- c("Expenditure per capita, market exchange rate-based (US$)")
# Population
# "Expenditure, market exchange rate-based (US$, billions)"
# "Expenditure per capita, PPP-based (US$)"
df_engelOther <- subset(df_engelOther, Element %in% element_vec)
df_gdpPcap <- subset(df_engelOther[, c("Country", "Good", "Value")], Good %in% c("1000000:GROSS DOMESTIC PRODUCT")) #"SP.POP.TOTL.ICP:Population"
#df_gdpPcap <- df_gdpPcap %>% spread(Good, Value)
colnames(df_gdpPcap)[ncol(df_gdpPcap)] <- c("GDP/capita (USD mkt. exchange rate based)")#, "Population")
df_gdpPcap$`GDP/capita (USD mkt. exchange rate based), logged` <- log(df_gdpPcap$`GDP/capita (USD mkt. exchange rate based)`)
# df_gdpPcap$`GDP (USD mkt. exchange rate based), logged` <- log(df_gdpPcap$`GDP (USD mkt. exchange rate based)`)
df_gdpPcap$Good <- NULL
df_engelOther <- subset(df_engelOther, !(Good %in% c("1000000:GROSS DOMESTIC PRODUCT")))
df_engelOther$Value <- log(df_engelOther$Value)
colnames(df_engelOther)[ncol(df_engelOther)] <- "Expenditure/capita (USD mkt. exchange rate based), logged"
df_engelOther <- merge(df_engelOther, df_gdpPcap, by = "Country")
ind_rm <- which(is.na(df_engelOther$`GDP/capita (USD mkt. exchange rate based), logged`))
df_engelOther <- df_engelOther[-ind_rm, ]





# Get region data (and alternate source of GDP/capita data)
this_folder <- "C:/Users/bensc/OneDrive/Documents/Data/FAO Data/"
...
df_gdp <- FAOdat_createRegionGroups(df_gdp, exclude_these = NULL,
                                      keep_FAOregions = F,
...
df_gdp$Area[grep("Tanzania", df_gdp$Area)] <- "Tanzania"
df_gdp$Area[grep("CÃ´te d'Ivoire", df_gdp$Area)] <- "Côte d'Ivoire"
#---
AsiaExclWAsia <- c("Southern Asia", "South-Eastern Asia", "Eastern Asia",
                   "Central Asia")
EurNAmAusNZ <- c("W/N/S Europe", "Eastern Europe", "North America", "Aus/NZ/Oceania")
NAfWAsia <- c("Northern Africa", "Western Asia")
df_gdp$Region[which(df_gdp$Region %in% AsiaExclWAsia)] <- "Asia\n(excluding W. Asia)"
df_gdp$Region[which(df_gdp$Region %in% EurNAmAusNZ)] <- "Europe / N. Amer.\nAustralia / NZ"
df_gdp$Region[which(df_gdp$Region %in% NAfWAsia)] <- "N. Africa / W. Asia"
#unique(df_gdp$Region)
#---
df_gdp$Value <- log(df_gdp$Value)
colnames(df_gdp)[c(1, 3)] <- c("Country", "GDP/capita (USD, 2015 prices), logged")
rm(df_raw); gc()


setdiff(df_gdp$Country, df_engelOther$Country)
setdiff(df_engelOther$Country, df_gdp$Country)
df_gdp$Country[grep("Venezuela", df_gdp$Country)] <- "Venezuela"



df_plot <- merge(df_engelOther, df_gdp, by = "Country")
#df_plot <- df_engelOther
keep_these <- c("1111000:RESTAURANTS AND HOTELS", "9120000:ACTUAL EDUCATION",
...
df_plot <- subset(df_plot, Good %in% keep_these)
...
# df_mod <- 
# df_mod <- df_mod[, c("Expenditure (% of GDP), logged", "GDP/capita (USD, 2015 prices), logged")]
# 
# mod <- lm(`Expenditure (% of GDP), logged` ~., df_mod)
# summary(mod)
# plot(mod$fitted.values, mod$residuals)
# ind_rm <- which(mod$residuals < -3)
# df_mod <- df_mod[-ind_rm, ]



gg <- ggplot(df_plot, aes(x = `GDP/capita (USD mkt. exchange rate based), logged`,
                          y = `Expenditure/capita (USD mkt. exchange rate based), logged`))
gg <- gg + geom_point()
...
adjR2_vec <- c()
for(i in 1:length(good_vec)){
  this_good <- good_vec[i]
  this_df_plot <- subset(df_plot, Good == this_good)
  mod <- lm(`Expenditure/capita (USD mkt. exchange rate based), logged` ~ `GDP/capita (USD mkt. exchange rate based), logged`, this_df_plot)
  #summary(mod)
  m <- round(mod$coefficients[2], 2)
  b <- round(mod$coefficients[1], 2)
  df_out <- as.data.frame(broom::glance(mod))
...
gg <- gg + scale_shape_manual(values = shape_vec)
#gg <- gg + geom_smooth(method = lm, se = F)
gg <- gg + facet_wrap(~Good, labeller = labeller(Good = facet_labels))
#gg <- gg + geom_text_repel(color = "black", size = smallLabel_size)
gg <- gg + theme(strip.background = element_blank(),
                 strip.text.x = element_text(hjust = -0.01, size = facetTitle_size),
                 #legend.direction = "vertical",
